...
'''
import re
passwords = ["5-10 b: bhbjlkbbbbbbb",
             "3-4 j: hjvj",
             "8-9 p: pmljtsttp",
...
for password in passwords:
    search = re.search("(\\d+)-(\\d+) (\\D): (.*)", password)
    if search is None:
        pass

    minVal, maxVal, letter, psk = search.group(1), search.group(
        2), search.group(3), search.group(4)

    if len(psk) >= int(maxVal) - 1 or len(psk) <= int(minVal)-1:
        pass

    if psk[int(minVal)-1] == letter and psk[int(maxVal)-1] != letter or psk[int(minVal)-1] != letter and psk[int(maxVal)-1] == letter:
        count += 1
